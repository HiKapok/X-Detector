...
def batch_norm_only(inputs, is_training, data_format):
  """Performs a batch normalization followed by a ReLU."""
  # We set fused=True for a significant performance boost. See
  # https://www.tensorflow.org/performance/performance_guide#common_fused_ops
  inputs = tf.layers.batch_normalization(
...
def conv2d_fixed_padding(inputs, filters, kernel_size, strides, data_format, kernel_initializer=tf.variance_scaling_initializer):
...
def xdet_bottleneck_block(inputs, filters, is_training, projection_shortcut,
  ...
  inputs = conv2d_fixed_padding(
      inputs=inputs, filters=filters, kernel_size=1, strides=1,
      data_format=data_format, kernel_initializer=tf.variance_scaling_initializer)
...
def imagenet_resnet_v2_generator(block_fn, layers, num_classes,
  ...
  def model(inputs, is_training):
    ...
    inputs = xdet_block_layer(
        ...
        data_format=data_format)
    # with tf.variable_scope('xdet_additional_conv', default_name = None, values = [inputs], reuse=tf.AUTO_REUSE):
    #   inputs = xdet_block_layer(
    #       inputs=inputs, filters=512, block_fn=xdet_bottleneck_block, blocks=layers[4],
    ...
    inputs = batch_norm_relu(inputs, is_training, data_format)
    inputs = tf.layers.average_pooling2d(
        inputs=inputs, pool_size=28, strides=1, padding='VALID',
        data_format=data_format)
    inputs = tf.identity(inputs, 'final_avg_pool')
...
def imagenet_resnet_v2(resnet_size, num_classes, data_format=None):
  ...
  model_params = {
      ...
      50: {'block': bottleneck_block, 'layers': [3, 4, 6, 3, 3]},
