...
def SEBlock(inputs, filters, data_format, is_training, rate = 8):
    ...
    # ( input_iamge_size / 8 - (7 - 1) ) / 4, use suitable input_iamge_size to produce integer here
    #inputs = resnet_v2.batch_norm_relu(inputs, is_training, data_format)
    ...
    outputs = tf.image.resize_images(inputs, input_shape * 4 + (7 - 1))
    if data_format == 'channels_first':
...
#           inputs=output_conv5, filters=1024, block_fn=xdet_bottleneck_block, blocks=layers[4],
#           dilation_rate=8, is_training=is_training, name='block_layer5',
#           data_format=data_format)
#     with tf.variable_scope('xdet_cls_seblock', default_name = None, values = [output_conv4, output_conv5, output_conv6], reuse=tf.AUTO_REUSE):
#       if data_format == 'channels_first':
#         cls_seblock_output = tf.concat([SEBlock(output_conv4, 256, data_format, is_training), SEBlock(output_conv5, 512, data_format, is_training), SEBlock(output_conv6, 1024, data_format, is_training)], axis = 1)
#       else:
#         cls_seblock_output = tf.concat([SEBlock(output_conv4, 256, data_format, is_training), SEBlock(output_conv5, 512, data_format, is_training), SEBlock(output_conv6, 1024, data_format, is_training)], axis = -1)
#     with tf.variable_scope('xdet_regress_seblock', default_name = None, values = [output_conv4, output_conv5, output_conv6], reuse=tf.AUTO_REUSE):
#       if data_format == 'channels_first':
#         regress_seblock_output = tf.concat([SEBlock(output_conv4, 256, data_format, is_training), SEBlock(output_conv5, 512, data_format, is_training), SEBlock(output_conv6, 1024, data_format, is_training)], axis = 1)
#       else:
#         regress_seblock_output = tf.concat([SEBlock(output_conv4, 256, data_format, is_training), SEBlock(output_conv5, 512, data_format, is_training), SEBlock(output_conv6, 1024, data_format, is_training)], axis = -1)
...
#       inputs = resnet_v2.batch_norm_only(concat_output, is_training, data_format)
#       return tf.nn.relu(inputs * cls_seblock_output), tf.nn.relu(inputs * regress_seblock_output)
