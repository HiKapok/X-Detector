...
def ssd_random_sample_patch_v0(image, labels, bboxes, name=None):
    ...
    def check_roi_overlap(width, height, bboxes, min_iou):
        index = 0
        max_attempt = 20
        roi = [0., 0., 0., 0.]
        ...
        def condition(index, roi):
            return tf.logical_or(tf.logical_and(tf.reduce_sum(tf.cast(jaccard_with_anchors(roi, bboxes) < min_iou, tf.int32)) > 0, tf.less(index, max_attempt)), tf.less(index, 1))
        ...
        [index, roi] = tf.while_loop(condition, body, [index, roi])
        return roi
    def check_roi_center(width, height, labels, bboxes, min_iou):
        index = 0
        max_attempt = 20
        roi = [0., 0., 0., 0.]
        ...
        def body(index, roi, mask):
            roi = check_roi_overlap(width, height, bboxes, min_iou)
        ...
        [index, roi, mask] = tf.while_loop(condition, body, [index, roi, mask])

        return tf.cast([roi[0]*tf.cast(height, tf.float32), roi[1]*tf.cast(width, tf.float32), (roi[2]-roi[0])*tf.cast(height, tf.float32), (roi[3]-roi[1])*tf.cast(width, tf.float32)], tf.int32), mask
    ...
    def sample_patch(image, labels, bboxes, min_iou):
        ...
        roi_slice_range, bboxes_mask = check_roi_center(width, height, labels, bboxes, min_iou)
        mask_labels = tf.boolean_mask(labels, bboxes_mask)
        mask_bboxes = tf.boolean_mask(bboxes, bboxes_mask)
