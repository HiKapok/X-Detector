...
def tf_summary_image(image, bboxes, name='image', unwhitened=False):
    ...
    tf.summary.image(name, image_with_box, max_outputs=3)
...
def distorted_bounding_box_crop(image,
    ...
    with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bboxes]):
        ...
        # bboxes = tf.Print(bboxes, [distort_bbox, bboxes], message='before bboxes_resize: ', summarize=20)
        # Update bounding boxes: resize and filter out.
        bboxes = bboxes_resize(distort_bbox, bboxes)

        # bboxes = tf.Print(bboxes, [distort_bbox, bboxes], message='after bboxes_resize: ', summarize=20)

        # filter by overlap, keep at least one
...
def light_head_preprocess_for_test(image, out_shape, data_format='NHWC', resize=Resize.WARP_RESIZE,
                            scope='light_head_preprocessing_test'):
    with tf.name_scope(scope):
        if image.get_shape().ndims != 3:
            raise ValueError('Input must be of size [height, width, C>0]')
...
def get_image_to_show(image, bboxes, unwhitened=False):
    ...
    If unwhitened is False, then the whole image must has pixel value between 0 - 1
    Else the input image must be subtracted by the mean of RGB, thus has pixel value between -128 - 128
    """
    image = tf.expand_dims(image, 0)
    bboxes = tf.expand_dims(bboxes, 0)

    if unwhitened:
        return tf_image_unwhitened(tf.image.draw_bounding_boxes(image, bboxes))

    return tf.cast(tf.squeeze(tf.image.draw_bounding_boxes(image, bboxes) * 255., 0), tf.int32)

def preprocess_for_unittest(image, labels, bboxes,
                             out_shape, data_format='NHWC',
                             scope='common_preprocessing_unittest'):

    fast_mode = False
    with tf.name_scope(scope, 'common_preprocessing_unittest', [image, labels, bboxes]):
        if image.get_shape().ndims != 3:
            raise ValueError('Input must be of size [height, width, C>0]')
        # Convert to float scaled [0, 1].
        if image.dtype != tf.float32:
            image = tf.image.convert_image_dtype(image, dtype=tf.float32)
        original_image_with_bbox = get_image_to_show(image, bboxes)

        # Randomly flip the image horizontally.
        image, bboxes = control_flow_ops.cond(tf.random_uniform([1], minval=0., maxval=1., dtype=tf.float32)[0] < 0.5, lambda: (image, bboxes), lambda: tf_image.ssd_random_expand(image, bboxes, 2))

        random_sample_image, labels, bboxes = tf_image.ssd_random_sample_patch(image, labels, bboxes)
        random_sample_resized_image = tf_image.resize_image(random_sample_image, out_shape,
                                          method=tf.image.ResizeMethod.BILINEAR,
                                          align_corners=False)

        random_sample_resized_image_with_bbox = get_image_to_show(random_sample_resized_image, bboxes)

        return original_image_with_bbox, random_sample_resized_image_with_bbox, labels

def preprocess_image_unittest(image,
                     labels,
                     bboxes,
                     out_shape,
                     data_format,
    ...
    return preprocess_for_unittest(image, labels, bboxes,
                                out_shape=out_shape,
