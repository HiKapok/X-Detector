    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope(FLAGS.model_scope, default_name=None, values=[features], reuse=tf.AUTO_REUSE):
            with tf.device('/gpu:0'):
                backbone = xdet_body_v4.xdet_resnet_v4(FLAGS.resnet_size, FLAGS.data_format)
                backbone_outputs = backbone(inputs=features, is_training=False)

                cls_pred, location_pred = xdet_body_v4.xdet_head(backbone_outputs, FLAGS.num_classes, all_num_anchors_depth, False, data_format=FLAGS.data_format)
        ...
        with tf.Session(config=config) as sess:
            ...
            for ind, image_name in enumerate(all_images):
                ...
                imsave(os.path.join(FLAGS.debug_dir, 'output_{}.jpg'.format(image_name[-10:-4])), img_to_draw)
...
# VOC07 metric? Yes
# AP for sofa = 0.8029
# AP for motorbike = 0.8208
# AP for train = 0.8691
# AP for person = 0.7632
# AP for aeroplane = 0.7840
# AP for boat = 0.6998
# AP for dog = 0.8577
# AP for horse = 0.8615
# AP for sheep = 0.7689
# AP for bicycle = 0.8247
# AP for car = 0.8025
# AP for diningtable = 0.7467
# AP for chair = 0.5799
# AP for pottedplant = 0.4755
# AP for cow = 0.7919
# AP for cat = 0.8681
# AP for bird = 0.7572
# AP for tvmonitor = 0.7235
# AP for bus = 0.8188
# AP for bottle = 0.4121
# Mean AP = 0.7514

# INFO:tensorflow:global_step/sec: 1.52384
# INFO:tensorflow:l2=1.926671, loss=3.608096, lr=0.000100, ce=1.344681, loc=0.336744, acc=0.886631
